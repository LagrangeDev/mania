...
use crate::core::event::login::{build_wtlogin_packet, parse_wtlogin_packet};
...
impl ClientEvent for WtLogin {
    ...
    fn parse(packet: Bytes, sess: &Session) -> CEParseResult {
        let packet = parse_wtlogin_packet(packet, sess)?;
        ...
        if typ == 0 {
            let enc_tlvs_data = original_tlvs
                ...
                .encrypted_tlv
                .to_owned();
            let dec_tlvs_data = tea_decrypt(&enc_tlvs_data, &sess.stub.tgtgt_key.load());
            ...
            sess.stub.tgtgt_key.store(Arc::from(tgtgt));
            sess.key_store.wt_session.tgt.store(Arc::from(tgt));
            sess.key_store.wt_session.d2.store(Arc::from(d2));
            sess.key_store
                .wt_session
                .d2_key
                .store(Arc::from(trans_d2_key.to_owned()));
            sess.key_store.uid.store(Some(Arc::from(uid)));
            sess.key_store
                .wt_session
                .temp_password
            ...
            sess.key_store.info.store(Arc::from(AccountInfo {
                age: self_info.age,
