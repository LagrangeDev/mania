...
impl SsoPacket {
        ...
        let body = PacketBuilder::new()
            .section(|header| {
                header
                    ...
                    .section(|p| p.bytes(&ctx.session.key_store.wt_session.tgt.load())) // tgt
        ...
        PacketBuilder::new()
            .section(|packet| {
                packet
                    ...
                    .u8(if ctx.session.key_store.wt_session.d2.load().is_empty() {
                        2
                    } else {
                        1
                    }) // flag
                    .section(|p| p.bytes(&ctx.session.key_store.wt_session.d2.load()))
                    .u8(0) // unknown
                    .section(|packet| packet.string(&ctx.session.key_store.uin.to_string())) // uin
                    .bytes(&tea::tea_encrypt(
                        &body,
                        ctx.session.key_store.wt_session.d2_key.load().as_ref(),
                    ))
            })
            .build()
    }
                ...
                packet
                    ...
                    .section(|p| {
                        p.section(|p| p.string(&self.command)) // command
                            .section(|p| p) // unknown
                            .section(|p| {
                                p.bytes(&if let Some(uid) =
                                    ctx.session.key_store.uid.load().as_deref()
                                {
                                    let uid = NtPacketUid {
                                        uid: Some(uid.clone()),
                                    };
    ...
    pub fn parse(packet: Bytes, sess: &Session) -> Result<SsoPacket, PacketError> {
        ...
        if uin != **sess.key_store.uin.load() && protocol == PacketType::T12 {
            return Err(PacketError::UinMismatch(**sess.key_store.uin.load(), uin));
        }
        ...
        let body = match auth_flag {
            0 => body,
            1 => tea::tea_decrypt(&body, sess.key_store.wt_session.d2_key.load().as_ref()).into(),
