...
impl KeyStore {
    ...
    pub fn is_session_expired(&self) -> bool {
        let now = Utc::now();
        let duration = now.signed_duration_since(**self.wt_session.session_date.load());
        duration.num_seconds() > 15 * 86400
    }

    pub fn is_expired(&self) -> bool {
        self.is_session_expired()
            || self.wt_session.d2.load().is_empty()
            || self.wt_session.tgt.load().is_empty()
    }
}
