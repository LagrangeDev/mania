...
impl MessagePacker {
    ...
    pub(crate) fn parse_chain(
        ...
        if let Some(grp) = &response_head.grp {
            let (mut ex_gf, mut ex_info) = match sess.config.fetch_group_member_strategy {
            ...
            return Ok(dda!(MessageChain {
                typ: MessageType::Group(GroupMessageUniqueElem {
                    ...
                    group_member_info: match sess.config.fetch_group_member_strategy {
                        FetchGroupMemberStrategy::Simple => {
    ...
    pub(crate) fn parse_fake_chain(
        ...
        let mut chain = MessagePacker::parse_chain(body, sess)?;
        chain.typ = typ;
        Ok(chain)
    }
    ...
    pub(crate) fn parse_private_file(
        ...
        let mut base_chain = MessagePacker::parse_chain(body, sess)?;
