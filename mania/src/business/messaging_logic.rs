use crate::business::{BusinessError, BusinessHandle, BusinessResult, LogicFlow};
...
use mania_core::core::event::message::push_msg::PushMessageEvent;
use mania_core::core::event::message::send_message::SendMessageEvent;
use mania_core::core::event::notify::bot_sys_rename::BotSysRenameEvent;
use mania_core::core::event::notify::friend_sys_new::FriendSysNewEvent;
use mania_core::core::event::notify::friend_sys_poke::FriendSysPokeEvent;
use mania_core::core::event::notify::friend_sys_recall::FriendSysRecallEvent;
use mania_core::core::event::notify::friend_sys_rename::FriendSysRenameEvent;
use mania_core::core::event::notify::friend_sys_request::FriendSysRequestEvent;
use mania_core::core::event::notify::group_sys_admin::GroupSysAdminEvent;
use mania_core::core::event::notify::group_sys_decrease::GroupSysDecreaseEvent;
use mania_core::core::event::notify::group_sys_essence::GroupSysEssenceEvent;
use mania_core::core::event::notify::group_sys_increase::GroupSysIncreaseEvent;
use mania_core::core::event::notify::group_sys_invite::GroupSysInviteEvent;
use mania_core::core::event::notify::group_sys_member_enter::GroupSysMemberEnterEvent;
use mania_core::core::event::notify::group_sys_member_mute::GroupSysMemberMuteEvent;
use mania_core::core::event::notify::group_sys_mute::GroupSysMuteEvent;
use mania_core::core::event::notify::group_sys_name_change::GroupSysNameChangeEvent;
use mania_core::core::event::notify::group_sys_pin_change::GroupSysPinChangeEvent;
use mania_core::core::event::notify::group_sys_poke::GroupSysPokeEvent;
use mania_core::core::event::notify::group_sys_reaction::GroupSysReactionEvent;
use mania_core::core::event::notify::group_sys_recall::GroupSysRecallEvent;
use mania_core::core::event::notify::group_sys_request_invitation::GroupSysRequestInvitationEvent;
use mania_core::core::event::notify::group_sys_request_join::GroupSysRequestJoinEvent;
use mania_core::core::event::notify::group_sys_special_title::GroupSysSpecialTitleEvent;
use mania_core::core::event::notify::group_sys_todo::GroupSysTodoEvent;
use mania_core::core::event::prelude::*;
use mania_core::entity::bot_group_member::FetchGroupMemberStrategy;
use mania_core::message::chain::{MessageChain, MessageType};
use mania_core::message::entity::Entity;
use mania_core::message::entity::file::FileUnique;
        ...
        match *entity {
            ...
            Entity::MultiMsg(ref mut multi)
                ...
                if multi.chains.is_empty() => {
                    let msg = handle
                        .download_multi_msg(chain.uid.clone(), multi.res_id.clone())
                        .await;
                    match msg {
                        Ok(Some(chains)) => {
                            multi.chains = chains;
    ...
    for entity in entities {
        match entity {
            Entity::Image(image) => match &chain.typ {
                MessageType::Group(grp) => handle.upload_group_image(grp.group_uin, image).await?,
                MessageType::Friend(_) | MessageType::Temp => {
                    handle.upload_c2c_image(&chain.uid, image).await?
                }
                _ => {}
            },
            Entity::Video(video) => match &chain.typ {
                MessageType::Group(grp) => handle.upload_group_video(grp.group_uin, video).await?,
                MessageType::Friend(_) | MessageType::Temp => {
                    handle.upload_c2c_video(&chain.uid, video).await?
                }
                _ => {}
            },
            Entity::Record(record) => match &chain.typ {
                MessageType::Group(grp) => {
                    handle.upload_group_record(grp.group_uin, record).await?
                }
                MessageType::Friend(_) | MessageType::Temp => {
                    handle.upload_c2c_record(&chain.uid, record).await?
                }
                _ => {}
            },
            _ => {}
...
async fn resolve_chain_metadata(
    ...
    match chain.typ {
        MessageType::Group(ref mut grp)
            if handle.context.session.config.fetch_group_member_strategy
                == FetchGroupMemberStrategy::Full =>
        {
            ...
            let lookup_uin = if chain.friend_uin == 0 {
                **handle.context.session.key_store.uin.load()
            } else {
                chain.friend_uin
            };
