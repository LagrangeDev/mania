...
use mania_core::core::event::login::wt_login::WtLogin;
use mania_core::core::event::login::wt_trans_emp::{
    NTLoginHttpRequest, NTLoginHttpResponse, TransEmp, TransEmp12Res, TransEmpResult,
};
use mania_core::core::event::system::alive::AliveEvent;
use mania_core::core::event::system::info_sync::InfoSyncEvent;
use mania_core::core::event::system::nt_sso_alive::NtSsoAliveEvent;
use mania_core::core::key_store::KeyStore;
use mania_core::core::session::QrSign;
...
impl BusinessHandle {
    pub fn update_key_store(&self) -> &KeyStore {
        &self.context.session.key_store
    }

    pub async fn fetch_qrcode(self: &Arc<Self>) -> BusinessResult<(String, Bytes)> {
        let mut trans_emp = TransEmp::new_fetch_qr_code();
        let response = self.send_event(&mut trans_emp).await?;
        let event: &TransEmp =
            downcast_major_event(&response).ok_or(BusinessError::InternalEventDowncastError)?;
        let result = event
            .result
            .as_ref()
            .ok_or_else(|| BusinessError::GenericError("Emp result not found".into()))?;
        if let TransEmpResult::Emp31(emp31) = result {
            let qr_sign = QrSign {
                sign: emp31
                    ...
                    .as_ref()
                    .try_into()
                    .map_err(|_| BusinessError::GenericError("invalid QR signature".into()))?,
    ...
    async fn query_trans_tmp_status(self: &Arc<Self>) -> BusinessResult<TransEmp12Res> {
        if let Some(qr_sign) = (*self.context.session.qr_sign.load()).clone() {
            ...
            self.context.session.key_store.uin.store(info.uin.into());
            ...
            Err(BusinessError::GenericError("QR sign not found".into()))
        }
    }

    async fn do_wt_login(self: &Arc<Self>) -> BusinessResult<()> {
        ...
        match event.code {
            0 => {
                tracing::info!(
                    ...
                    self.context.session.key_store.info
                );
                Ok(())
            }
            _ => Err(BusinessError::GenericError(
                format!(
    ...
    pub async fn login_by_qrcode(self: &Arc<Self>) -> BusinessResult<()> {
            ...
            loop {
                ...
                match status {
                    ...
                    TransEmp12Res::CodeExpired => {
                        return Err(BusinessError::GenericError("QR code expired".into()));
                    }
                    TransEmp12Res::Canceled => {
                        return Err(BusinessError::GenericError(
        ...
        match result {
            Ok(Ok(())) => Ok(()),
            Ok(Err(e)) => Err(e),
            Err(_) => Err(BusinessError::GenericError(
                "QR code scan timed out after 120s!".into(),
            )),
        }
    }

    pub async fn online(self: &Arc<Self>) -> BusinessResult<watch::Sender<()>> {
        ...
        tracing::debug!(
            "d2key: {:?}",
            (**self.context.session.key_store.wt_session.d2_key.load()).hex()
        );
