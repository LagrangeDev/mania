...
use crate::highway::{HighwayError, oidb_ipv4s_to_highway_ipv4s};
use bytes::Bytes;
...
use mania_core::core::crypto::stream_sha1::StreamSha1;
use mania_core::core::event::downcast_major_event;
use mania_core::core::event::message::image_c2c_upload::{ImageC2CUploadArgs, ImageC2CUploadEvent};
use mania_core::core::event::message::image_group_upload::{
    ImageGroupUploadArgs, ImageGroupUploadEvent,
};
use mania_core::core::event::message::record_c2c_upload::{
    RecordC2CUploadArgs, RecordC2CUploadEvent,
};
use mania_core::core::event::message::record_group_upload::{
    RecordGroupUploadArgs, RecordGroupUploadEvent,
};
use mania_core::core::event::message::video_c2c_upload::{VideoC2CUploadArgs, VideoC2CUploadEvent};
use mania_core::core::event::message::video_group_upload::{
    VideoGroupUploadArgs, VideoGroupUploadEvent,
};
use mania_core::core::event::system::fetch_highway_ticket::FetchHighwayTicketEvent;
use mania_core::core::protos::service::highway::{
    NtHighwayHash, NtHighwayNetwork, Ntv2RichMediaHighwayExt,
};
use mania_core::core::stream::{AsyncPureStream, AsyncStream};
use mania_core::dda;
use mania_core::message::entity::image::ImageEntity;
use mania_core::message::entity::record::RecordEntity;
use mania_core::message::entity::video::VideoEntity;
use mania_core::utility::extensions::HexString;
use mania_core::utility::image_resolver::{ImageFormat, resolve_image_metadata};
use mania_core::utility::stream_helper::{mut_stream_ctx, stream_pipeline};
use md5::Md5;
use prost::Message;
...
impl BusinessHandle {
    async fn fetch_sig_session(self: &Arc<Self>) -> BusinessResult<Bytes> {
        let mut req = FetchHighwayTicketEvent::default();
        let req = self.send_event(&mut req).await?;
        let res: &FetchHighwayTicketEvent =
            downcast_major_event(&req).ok_or(BusinessError::InternalEventDowncastError)?;
    ...
    async fn prepare_highway(self: &Arc<Self>) -> BusinessResult<()> {
        ...
        self.highway.client.store(Arc::new(HighwayClient::new(
            ...
            60,
            sig,
            **self.context.session.key_store.uin.load(),
            self.context.session.config.highway_chuck_size,
        )));
        Ok(())
    }
    ...
    ) -> BusinessResult<((ImageFormat, u32, u32), Bytes, Bytes)> {
        let (iv, sha1_bytes, md5_bytes) = mut_stream_ctx(&stream_ctx, |s| {
            Box::pin(async move {
                ...
                let iv = resolve_image_metadata(s).await.map_err(|e| {
                    BusinessError::GenericError(Cow::from(format!("Resolve image error: {e:?}")))
                })?;
                let sha1_bytes = Bytes::from(sha1_hasher.finalize().to_vec());
                let md5_bytes = Bytes::from(md5_hasher.finalize().to_vec());
                Ok::<((ImageFormat, u32, u32), Bytes, Bytes), BusinessError>((
    ...
    pub async fn upload_group_image(
        ...
        let stream = image
            .resolve_stream()
            .await
            .ok_or(BusinessError::GenericError(Cow::from(
        ...
        let res: &ImageGroupUploadEvent =
            downcast_major_event(&res).ok_or(BusinessError::InternalEventDowncastError)?;
        if res.res.u_key.as_ref().is_some() {
            ...
            let size = image.size;
            let chunk_size = self.context.session.config.highway_chuck_size;
    ...
    async fn resolve_video(
    ...
    ) -> BusinessResult<(Bytes, Bytes, Vec<Vec<u8>>, Bytes, Bytes)> {
        let (file_md5, file_sha1, file_stream_sha1) = mut_stream_ctx(&video_stream_ctx, |s| {
            Box::pin(async move {
                ...
                Ok::<(Bytes, Bytes, Vec<Vec<u8>>), BusinessError>((md5, sha1, stream_sha1))
            })
        })
        .await?;
        let (thumb_md5, thumb_sha1) = mut_stream_ctx(&video_thumb_stream_ctx, |s| {
            Box::pin(async move {
                ...
                let md5 = Bytes::from(md5_hasher.finalize().to_vec());
                let sha1 = Bytes::from(sha1_hasher.finalize().to_vec());
                Ok::<(Bytes, Bytes), BusinessError>((md5, sha1))
            })
        })
        .await?;
    ...
    pub async fn upload_group_video(
        ...
        let (vs, is) = video.resolve_stream().await.map_err(|e| {
            BusinessError::GenericError(Cow::from(format!("Resolve stream error: {e:?}")))
        })?;
        let vs = vs.ok_or(BusinessError::GenericError(Cow::from(
            "No video stream found",
        )))?;
        let is = is.ok_or(BusinessError::GenericError(Cow::from(
            "No image stream found",
        )))?;
    ...
    async fn resolve_audio(
        self: &Arc<Self>,
        stream_ctx: AsyncStream,
    ) -> BusinessResult<(AsyncPureStream, u32, f64, Bytes, Bytes)> {
                ...
                Box::pin(async move {
                    ...
                    let task = tokio::task::spawn_blocking(|| {
                        ...
                        let pipeline = AudioRwStream::new(Box::new(cursor))
                            ...
                            .map_err(|e| {
                                BusinessError::GenericError(Cow::from(format!(
                                    "Decode error: {e:?}"
                                )))
                            })?
                        ...
                        Ok::<Vec<u8>, BusinessError>(output)
                    });
                    let res = task.await.map_err(|e| {
                        BusinessError::GenericError(Cow::from(format!("Blocking error: {e:?}")))
                    })??;
                    ...
                    let sha1_bytes = Bytes::from(sha1_hasher.finalize().to_vec());
                    let md5_bytes = Bytes::from(md5_hasher.finalize().to_vec());
                    Ok::<(AsyncPureStream, u32, f64, Bytes, Bytes), BusinessError>((
                        silk_pure_stream,
                        stream_len as u32,
                        time,
                        sha1_bytes,
